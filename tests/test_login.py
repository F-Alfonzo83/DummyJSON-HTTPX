...
def test_retrieve_token(dummyjson_client):
    auth_token = dummyjson_client.auth_client.retrieve_auth_token(username=os.getenv("DEFAULT_USER_NAME"),
                                                                  password=os.getenv("DEFAULT_USER_PASS"))
...
def test_auth_token_request_time_and_expires_setters(dummyjson_client):
    expiration_time = 25
    logger.debug("TEST: Setting initial timer")
    initial_time = time.time()
    dummyjson_client.auth_client.retrieve_auth_token(username=os.getenv("DEFAULT_USER_NAME"),
                                                     password=os.getenv("DEFAULT_USER_PASS"),
...
def test_negative_authenticate_invalid_user(dummyjson_client):
    response = dummyjson_client.auth_client.authenticate(username="Shawarma",
                                                         password=os.getenv("DEFAULT_USER_PASS"))
...
def test_negative_retrieve_auth_token_invalid_user(dummyjson_client):
    with pytest.raises(httpx.HTTPStatusError) as request_error_wrapper:
        dummyjson_client.auth_client.retrieve_auth_token(username=os.getenv("DEFAULT_USER_NAME")+"1",
                                                         password=os.getenv("DEFAULT_USER_PASS"))
...
def test_negative_retrieve_auth_token_invalid_password(dummyjson_client):
    with pytest.raises(httpx.HTTPStatusError) as request_error_wrapper:
        dummyjson_client.auth_client.retrieve_auth_token(username=os.getenv("DEFAULT_USER_NAME"),
                                                         password=os.getenv("DEFAULT_USER_PASS")+"1")
    assert_status_code(request_error_wrapper.value.response, 400)
    assert request_error_wrapper.value.response.json() == {"message": "Invalid credentials"}, \
        f"TEST FAIL: Error message does not match: {request_error_wrapper.value.response.json()}"
