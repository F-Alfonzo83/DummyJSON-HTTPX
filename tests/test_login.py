...
def test_login(dummyjson_client,  default_user_credentials):
    username,  password = default_user_credentials
    response = dummyjson_client.auth_client.authenticate(username=username,
                                                         password=password)
    ...
    assert schema_validation.username == username


def test_retrieve_token(dummyjson_client, default_user_credentials):
    username,  password = default_user_credentials
    auth_token = dummyjson_client.auth_client.retrieve_auth_token(username=username,
                                                                  password=password)
...
def test_auth_token_request_time_and_expires_setters(dummyjson_client, default_user_credentials):
    ...
    dummyjson_client.auth_client.retrieve_auth_token(username=username,
                                                     password=password,
...
def test_negative_authenticate_invalid_user(dummyjson_client, default_user_credentials):
    username,  password = default_user_credentials
    response = dummyjson_client.auth_client.authenticate(username=username+"1",
                                                         password=password)
...
def test_negative_retrieve_auth_token_invalid_user(dummyjson_client, default_user_credentials):
    ...
    with pytest.raises(httpx.HTTPStatusError) as request_error_wrapper:
        dummyjson_client.auth_client.retrieve_auth_token(username=username+"1",
                                                         password=password)
